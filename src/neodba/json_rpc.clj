...
(defn read-message
  ...
  (let [headers (h/read-headers)
        content-length-header (some #(when (str/starts-with? % "Content-Length") %)
                                    headers)
        content-length (if content-length-header
                         (u/parse-int (-> content-length-header (subs 15) str/trim))
                         0)]
    (if (pos-int? content-length)
      (let [body (-> (h/read-body content-length) json/read-str)]
